public class KdTree {
    ...
    public Point2D closest(Point2D point) {
        if (isEmpty()) {
            ...
            result = closest(root, point, result);

            return result;
        }
    }

    private Point2D closest(Node x, Point2D point, Point2D min) {
        if (x != null) {
            if (min == null) {
                min = x.p;
            }

            // Si el punto mínimo actual está más cerca de la consulta que el punto actual
            if (min.distanceSquaredTo(point)
                ...
                // Comprueba en que orden se debe iterar
                if (x.right != null && x.right.rect.contains(point)) {
                    min = closest(x.right, point, min);
                    min = closest(x.left, point, min);
                } else {
                    min = closest(x.left, point, min);
                    min = closest(x.right, point, min);
                }
