...
public final class GraphvizBuilder {
	...
	public static void binaryHeapToFile (double[] heap, int N) {
	...
	public static void binaryHeapToFile (double[] heap, int N, String filename) {
	...
	private void addBinaryHeap (Object[] heap, int N) {
		...
		for (int i = N+1; i<2<<logN; i++) {
			addFromNullEdgeString (prefix + (i/2));
		}
//		if (N%2==0) {
//			addFromNullEdgeString (prefix + (N/2));
//		}
	}
	private void addBinaryHeap (double[] heap, int N) {
		if (heap == null) throw new IllegalArgumentException ("null heap");
		if (N<0 || N>heap.length) throw new IllegalArgumentException ("N=" + N + " is not in range [0.." + heap.length + "]");
		String prefix = "heap__" + heap.hashCode ();
		for (int i = 1; i <= N; i++) {
			addLabeledNodeString (prefix + i, Double.toString(heap[i]));
		}
		for (int i = 2; i <= N; i++) {
			addEdgeString (prefix + i, prefix + (i/2));
		}
...
//		}
	...
	public void addNullEdge (Object src) { addNullEdgeString(getId (src)); }
	public void addNullEdgeString (String src) {
		String id = "null__" + nullId;
		nullId++;
		addLabeledNodeString (id, "", nullNodeProperties);
		if (src != null) addEdgeString (src, id, nullEdgeProperties);
	}
	public void addFromNullEdgeString (String src) {
		String id = "null__" + nullId;
		nullId++;
		addLabeledNodeString (id, "", nullNodeProperties);
		if (src != null) addEdgeString (id, src, nullEdgeProperties);
	}
	public void addLabeledNullEdge (Object src, String label) { addLabeledNullEdgeString(getId (src), label); }
	public void addLabeledNullEdgeString (String src, String label) {
		String id = "null__" + nullId;
		nullId++;
		addLabeledNodeString (id, "", nullNodeProperties);
		if (src != null) addLabeledEdgeString (src, id, getLabel (label), nullEdgeProperties);
	}
	 ...
	 * If baseDirName is a relative pathname, then it is placed in the users Desktop folder.
	...
	private static String baseDirName = "GraphvizOutput";
	...
	private static void setDirName () {
		...
		if (!dir.isAbsolute ()) {
			baseDirName = getDesktop() + File.separator + baseDirName;
