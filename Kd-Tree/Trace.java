...
public class Trace {
	 ...
	 * Relative pathnames are interpreted with respect to the user's "Desktop" directory.
	 * Default is "GraphvizOutput".
	 */
	public static void setGraphizOutputDir (String dirName) {
		GRAPHVIZ_DIR = dirName;
	}
	private static String GRAPHVIZ_DIR = "GraphvizOutput";
	...
	public static void setConsoleFilenameRelativeToUserDesktop (String filename) {
		setConsoleFilename (Graphviz.getDesktop() + File.separator + filename);
	}
	/**
...
//	}
	...
	public static void showObjectIdsRedundantly (boolean value) {
		Trace.showBuiltInObjects(value);
		GRAPHVIZ_SHOW_OBJECT_IDS = value;
	...
	private static VirtualMachine launchConnect (String[] args) {
		...
		Map<String, ? extends Argument> connArgs = null;

		if (Graphviz.isWindows()) {
			
			conn = Bootstrap.virtualMachineManager ().defaultConnector(); //"com.sun.jdi.CommandLineLaunch"
			try {
				connArgs = conn.defaultArguments ();
				connArgs.get ("main").setValue (argsString);
			} catch (NullPointerException e) {
				throw new Error ("\n!!!! Bad launching connector");
			}

		} else {
			// The default launcher works fine for windows, which uses shared memory
			// for communication between the processes.
			// The default launcher fails on unix-like systems, which uses sockets
			// for communication between the processes.
			// In particular, the communication is set up using hostname; this causes 
			// communication to fail when the hostname is not routable in DNS or /etc/hosts
			// The problem is in the file com/sun/tools/jdiSocketTransportService$SocketListenKey.java
			// To fix, you just need to comment out the try block with "address = InetAddress.getLocalHost ();"
			// As of Java 9, this fix is not available to outsiders, due to the module system
			//			
			// The use of the raw launcher here is reverse engineered from 
			//  com.sun.tools.jdi.SunCommandLineLauncher
			//  com.sun.tools.jdi.RawCommandLineLauncher
			//
			// The raw connector has these attributes:
			//   command = "" : Raw command to start the debugged application VM
			//   address = "" : Address from which to listen for a connection after the raw command is run
			//   quote = "\"" : Character used to combine space-delimited text into a single command line argument
			//
			// The default connector has these attributes:
			//   home = System.getProperty("java.home") : Home directory of the SDK or runtime environment used to launch the application
			...
			String command = 
					System.getProperty ("java.home") + fs + "bin" + fs + "java " +
							"-Xdebug " +
							"-Xrunjdwp:transport=dt_socket,address=127.0.0.1:8900,suspend=y " + 
							"-classpath \"" + System.getProperty ("java.class.path") + "\" " +							
							argsString +
							"";

			for (LaunchingConnector lc : Bootstrap.virtualMachineManager ().launchingConnectors ()) {
				if (lc.name ().equals ("com.sun.jdi.RawCommandLineLaunch")) conn = lc;
			}
			try {
				connArgs = conn.defaultArguments ();
				connArgs.get ("command").setValue (command);
				connArgs.get ("address").setValue ("127.0.0.1:8900");
			} catch (NullPointerException e) {
				throw new Error ("\n!!!! Bad launching connector");
			}
		}
...
/* private static */class Printer implements IndentPrinter, JDIEventHandler {
	...
	public void methodEntryEvent (MethodEntryEvent event) {
		...
		//System.err.println (Trace.GRAPHVIZ_CLASS_NAME);
...
/* private static */class Graphviz {
	...
	public static String getDesktop () {
		if (isWindows()) {
			// Supposedly this selects the windows desktop folder:
	...
	private static void setBaseFilename () {
		...
		if (!dir.isAbsolute ()) {
			dirName = getDesktop() + File.separator + dirName;
